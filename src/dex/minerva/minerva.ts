...
import { MinervaData, DexParams } from './types';
import { MinervaEventPool } from './pool';
import { SimpleExchange } from '../simple-exchange';
import { MinervaConfig, Adapters } from './config';
import { Vault } from './vault';
import ERC20ABI from '../../abi/erc20.json';

const MinervaGasCost = 300 * 1000;

export class Minerva extends SimpleExchange implements IDex<MinervaData> {
  protected pool: MinervaEventPool | null = null;
  ...
  public static dexKeysWithNetwork: { key: string; networks: Network[] }[] =
    getDexKeysWithNetwork(MinervaConfig);
  ...
  constructor(
    ...
    protected params: DexParams = MinervaConfig[dexKey][network],
  ) {
  ...
  async initializePricing(blockNumber: number) {
    ...
    this.pool = new MinervaEventPool(
      this.dexKey,
      this.network,
      this.dexHelper,
      this.logger,
  ...
  async getPricesVolume(
    ...
    blockNumber: number,
    limitPools?: string[],
  ): Promise<null | ExchangePrices<MinervaData>> {
  ...
  getCalldataGasCost(poolPrices: PoolPrices<MinervaData>): number | number[] {
  ...
  getSimpleParam(
    ...
    return {
      ...
      calldata: [
        Minerva.erc20Interface.encodeFunctionData('transfer', [
          this.params.vault,
          srcAmount,
        ]),
  ...
  async updatePoolState(): Promise<void> {
    if (!this.supportedTokens.length) {
      const tokenAddresses = await MinervaEventPool.getWhitelistedTokens(
        this.params.vault,
        'latest',
        this.dexHelper.multiContract,
      );
      ...
      const decimalsCallData =
        Minerva.erc20Interface.encodeFunctionData('decimals');
    ...
    const erc20BalanceCalldata = Minerva.erc20Interface.encodeFunctionData(
      'balanceOf',
      [this.params.vault],
    );
