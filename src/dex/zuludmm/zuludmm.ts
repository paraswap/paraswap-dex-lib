...
import { Adapters, ZuluDmmConfig } from './config';
import { Logger } from 'log4js';
import { Contract } from 'web3-eth-contract';

import zuluDmmFactoryABI from '../../abi/zuludmm/zulu-dmm-factory.abi.json';
import zuluDmmPoolABI from '../../abi/zuludmm/zulu-dmm-pool.abi.json';
import ZuluDmmExchangeRouterABI from '../../abi/zuludmm/zulu-dmm-exchange-router.abi.json';
import { getBigIntPow, getDexKeysWithNetwork } from '../../utils';

const MAX_TRACKED_PAIR_POOLS = 3;

const iface = new Interface(zuluDmmPoolABI);
const coder = new AbiCoder();

export class ZuluDmm
  extends SimpleExchange
  implements IDex<ZuluDmmData, ZuluDmmParam>
{
  pairs: { [key: string]: ZuluDmmPair } = {};
  needWrapNative = true;
  factory: Contract;
  logger: Logger;
  ...
  public static dexKeysWithNetwork: { key: string; networks: Network[] }[] =
    getDexKeysWithNetwork(ZuluDmmConfig);

  constructor(
    protected network: Network,
    dexKey: string,
    protected dexHelper: IDexHelper,
    protected config = ZuluDmmConfig[dexKey][network],
    ...
    this.factory = new this.dexHelper.web3Provider.eth.Contract(
      zuluDmmFactoryABI as any,
      config.factoryAddress,
    );

    this.exchangeRouterInterface = new Interface(ZuluDmmExchangeRouterABI);
  }
  ...
  async getSimpleParam(
    ...
    const swapFunctionParams: ZuluDmmParam = [
    ...
    const swapData = this.exchangeRouterInterface.encodeFunctionData(
      isSell
        ? ZuluDMMFunctions.swapExactTokensForTokens
        : ZuluDMMFunctions.swapTokensForExactTokens,
      swapFunctionParams,
    );
  ...
  async getPricesVolume(
    ...
    blockNumber: number,
    limitPools?: string[],
  ): Promise<ExchangePrices<ZuluDmmData> | null> {
    try {
      if (!this.factory) {
        return null;
      }
  ...
  getCalldataGasCost(poolPrices: PoolPrices<ZuluDmmData>): number | number[] {
    return (
  ...
  private async getPoolPrice(
    ...
    side: SwapSide,
    amounts: bigint[],
    pool: { poolAddress: string; state: ZuluDmmPoolState },
    direction: boolean,
  ...
  private async getManyPoolReserves(
    pair: ZuluDmmPair,
    blockNumber: number,
  ): Promise<{ [poolAddress: string]: ZuluDmmPoolState }> {
    try {
      ...
      return pair.exchanges.reduce(
        (acc: { [key: string]: ZuluDmmPoolState }, poolAddress, poolIndex) => {
  ...
  private async getPairOrderedParams(
    ...
    to: Token,
    blockNumber: number,
  ): Promise<ZuluDmmPoolOrderedParams | null> {
    const pair = await this.findPair(from, to);
    ...
    const pairState = Object.entries(pair.pools)
      .map(([poolAddress, pool]) => ({
        poolAddress,
        state: pool.getState(blockNumber) as ZuluDmmPoolState,
        ampBps: pool.ampBps,
      }))
      .filter(s => s.state);
