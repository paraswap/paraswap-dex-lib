...
import { ZuluDmmAbiEvents, TradeInfo } from './types';
import { StatefulEventSubscriber } from '../../stateful-event-subscriber';
import { Address, BlockHeader, Log, Logger, Token } from '../../types';
import { IDexHelper } from '../../dex-helper/idex-helper';
import { BI_POWS } from '../../bigint-constants';

export type ZuluDmmPools = { [poolAddress: string]: ZuluDmmPool };

export type ZuluDmmPair = {
  token0: Token;
  token1: Token;
  exchanges: Address[];
  pools: ZuluDmmPools;
...
export interface ZuluDmmPoolState {
  reserves: {
    reserves0: bigint;
    reserves1: bigint;
    vReserves0: bigint;
...
export interface ZuluDmmPoolOrderedParams {
  tokenIn: string;
  tokenOut: string;
  poolData: Array<{
    poolAddress: string;
    state: ZuluDmmPoolState;
  }>;
  direction: boolean;
  exchanges: string[];
}
...
export class ZuluDmmPool extends StatefulEventSubscriber<ZuluDmmPoolState> {
  decoder: (log: Log) => ZuluDmmAbiEvents = (log: Log) =>
    iface.parseLog(log) as any as ZuluDmmAbiEvents;
  ...
  protected async processLog(
    state: DeepReadonly<ZuluDmmPoolState>,
    log: Readonly<Log>,
    blockHeader: BlockHeader,
  ): Promise<DeepReadonly<ZuluDmmPoolState> | null> {
