...
import {
  ...
  RamsesV2Functions,
  RamsesV2Param,
  RamsesV2SimpleSwapParams,
} from './types';
import {
  getLocalDeadlineAsFriendlyPlaceholder,
  SimpleExchange,
} from '../simple-exchange';
import { RamsesV2Config, Adapters } from './config';
...
export class RamsesV2
  extends SimpleExchange
  implements IDex<RamsesV2Data, RamsesV2Param>
{
  ...
  public static dexKeysWithNetwork: { key: string; networks: Network[] }[] =
    getDexKeysWithNetwork(_.pick(RamsesV2Config, ['RamsesV2']));
  ...
  constructor(
    ...
    protected config = RamsesV2Config[dexKey][network],
  ) {
  ...
  async getPricingFromRpc(
    ...
    amounts: bigint[],
    side: SwapSide,
    pools: RamsesV2EventPool[],
  ): Promise<ExchangePrices<RamsesV2Data> | null> {
    if (pools.length === 0) {
      return null;
    }
  ...
  async getPricesVolume(
    ...
    try {
      ...
      let selectedPools: RamsesV2EventPool[] = [];
      ...
      const poolsToUse = selectedPools.reduce(
        ...
        {
          poolWithState: [] as RamsesV2EventPool[],
          poolWithoutState: [] as RamsesV2EventPool[],
        },
      );
      ...
      const notNullResult = result.filter(
        res => res !== null,
      ) as ExchangePrices<RamsesV2Data>;
  ...
  async preProcessTransaction(
    optimalSwapExchange: OptimalSwapExchange<RamsesV2Data>,
    srcToken: Token,
    _0: Token,
    _1: SwapSide,
    options: PreprocessTransactionOptions,
  ): Promise<[OptimalSwapExchange<RamsesV2Data>, ExchangeTxInfo]> {
  ...
  getDirectParam(
    ...
    const swapParams: RamsesV2Param = [
      srcToken,
      destToken,
      this.config.router,
      srcAmount,
    ...
    const encoder = (...params: RamsesV2Param) => {
  ...
  async getSimpleParam(
    ...
    const swapFunction =
      side === SwapSide.SELL
        ? RamsesV2Functions.exactInput
        : RamsesV2Functions.exactOutput;

    const path = this._encodePath(data.path, side);
    const swapFunctionParams: RamsesV2SimpleSwapParams =
      side === SwapSide.SELL
        ? {
  ...
  private async _getPoolsFromIdentifiers(
    ...
    return pools.filter(pool => pool) as RamsesV2EventPool[];
  }
