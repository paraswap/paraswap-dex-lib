...
import { MMath } from './maverick-sol-math';
...
export class MaverickPoolMath {
  ...
  constructor(
    ...
    this.ind_1_a2 = BI_POWS[18] - MMath.sMul(2n, this.lamb_w) + this.lamb2_w;
  ...
  _quadraticRoots(a: bigint, b: bigint, c: bigint) {
    const d = MMath.div(
      MMath.sqrt(MMath.mul(b, b) - MMath.sMul(4n, MMath.mul(a, c))),
      MMath.sMul(2n, a),
    );
    return MMath.max(0n, MMath.div(-b, MMath.sMul(2n, a)) + d);
  }
  ...
  _getAssetsInActiveBin() {
    ...
    if (activeInd == 1) {
      ...
      activeInfo.a1 =
        ...
        MMath.mul(
          _lamb_w,
          MMath.sMul(2n, this.state.quoteBalance) - activeInfo.c,
        ) -
        MMath.sMul(2n, quotelamb);
      ...
      activeInfo.a1 =
        ...
        MMath.mul(
          this.state.baseBalance,
          this.gam_w - MMath.sMul(2n, this.gam2_k),
        );
