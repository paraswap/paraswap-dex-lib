...
import { SquadswapV3Config, Adapters } from './config';
import { SquadswapV3EventPool } from './squadswap-v3-pool';
...
import {
  SQUADSWAPV3_EFFICIENCY_FACTOR,
  SQUADSWAPV3_TICK_BASE_OVERHEAD,
  SQUADSWAPV3_POOL_SEARCH_OVERHEAD,
  SQUADSWAPV3_TICK_GAS_COST,
} from './constants';
...
export class SquadswapV3 extends SimpleExchange implements IDex<UniswapV3Data> {
  private readonly factory: SquadswapV3Factory;
  readonly isFeeOnTransferSupported: boolean = false;
  readonly eventPools: Record<string, SquadswapV3EventPool | null> = {};
  ...
  public static dexKeysWithNetwork: { key: string; networks: Network[] }[] =
    getDexKeysWithNetwork(_.pick(SquadswapV3Config, ['SquadswapV3']));
  ...
  constructor(
    ...
    protected config = SquadswapV3Config['SquadswapV3'][network], // protected poolsToPreload = PoolsToPreload[dexKey][network] || [],
  ) {
    ...
    this.factory = new SquadswapV3Factory(
      dexHelper,
      dexKey,
      this.config.factory,
      this.logger,
  ...
  async getPricesVolume(
    ...
    try {
      ...
      let selectedPools: SquadswapV3EventPool[] = [];
      ...
      const poolsToUse = selectedPools.reduce(
        ...
        {
          poolWithState: [] as SquadswapV3EventPool[],
          poolWithoutState: [] as SquadswapV3EventPool[],
        },
      );
            ...
            ...pricesResult.outputs.map((p, index) => {
              if (p == 0n) {
                ...
                return (
                  SQUADSWAPV3_POOL_SEARCH_OVERHEAD +
                  SQUADSWAPV3_TICK_BASE_OVERHEAD +
                  pricesResult.tickCounts[index] * SQUADSWAPV3_TICK_GAS_COST
                );
              }
            }),
          ];
  ...
  async getTopPoolsForToken(
    ...
    const pools0 = _.map(res.pools0, pool => ({
      ...
      liquidityUSD:
        parseFloat(pool.totalValueLockedUSD) * SQUADSWAPV3_EFFICIENCY_FACTOR,
    }));
  ...
  private async _getPoolsFromIdentifiers(
    ...
    return pools.filter(pool => pool) as SquadswapV3EventPool[];
  }
