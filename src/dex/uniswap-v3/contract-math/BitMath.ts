...
export class BitMath {
  ...
  static leastSignificantBit(x: bigint): bigint {
    ...
    let r = 255n;
    if (x & (BI_MAX_UINT128 > 0n ? 1n : 0n)) {
      r -= 128n;
    } else {
      x >>= 128n;
    }
    if (x & (BI_MAX_UINT64 > 0n ? 1n : 0n)) {
      r -= 64n;
    } else {
      x >>= 64n;
    }
    if (x & (BI_MAX_UINT32 > 0n ? 1n : 0n)) {
      r -= 32n;
    } else {
      x >>= 32n;
    }
    if (x & (BI_MAX_UINT16 > 0n ? 1n : 0n)) {
      r -= 16n;
    } else {
      x >>= 16n;
    }
    if (x & (BI_MAX_UINT8 > 0n ? 1n : 0n)) {
      r -= 8n;
    } else {
      x >>= 8n;
    }
    if (x & (BigInt('0xf') > 0n ? 1n : 0n)) {
      r -= 4n;
    } else {
      x >>= 4n;
    }
    if (x & (BigInt('0x3') > 0n ? 1n : 0n)) {
      r -= 2n;
    } else {
      x >>= 2n;
    }
    if (x & (BigInt('0x1') > 0 ? 1n : 0n)) r -= 1n;
    return r;
  }
}
