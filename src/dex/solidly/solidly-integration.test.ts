...
import { Glacier } from './forks-override/glacier';
...
describe('Solidly integration tests', () => {
  ...
  describe('Avalanche', () => {
    ...
    describe('Glacier', function () {
      ...
      const glacier = new Glacier(network, dexKey, dexHelper);

      describe('UniswapV2 like pool', function () {
        const TokenASymbol = 'WAVAX';
        const tokenA = Tokens[network][TokenASymbol];
        const TokenBSymbol = 'BTCb';
        const tokenB = Tokens[network][TokenBSymbol];
        ...
        it('getPoolIdentifiers and getPricesVolume', async function () {
          const blocknumber = await dexHelper.web3Provider.eth.getBlockNumber();
          const pools = await glacier.getPoolIdentifiers(
            tokenA,
            tokenB,
            SwapSide.SELL,
            blocknumber,
          ...
          const poolPrices = await glacier.getPricesVolume(
            tokenA,
            tokenB,
            amounts,
            SwapSide.SELL,
        ...
        it('getTopPoolsForToken', async function () {
          const poolLiquidity = await glacier.getTopPoolsForToken(
            tokenA.address,
            10,
          );
          console.log(`${TokenASymbol} Top Pools:`, poolLiquidity);
