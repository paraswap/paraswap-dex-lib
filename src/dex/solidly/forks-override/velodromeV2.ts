...
export class VelodromeV2 extends Solidly {
  ...
  async initPools(fromIndex: number, toIndex: number) {
    ...
    for (let i = fromIndex; i < toIndex; i++) {
      allPoolsCallData.push({
        target: this.factoryAddress,
        callData: velodromeV2FactoryIface.encodeFunctionData('allPools', [i]),
        decodeFunction: addressDecode,
      });
    }
      ...
      poolsCalldata.push(
        {
          ...
          callData: velodromeV2FactoryIface.encodeFunctionData('token0', []),
          decodeFunction: addressDecode,
        },
        {
          target: poolResult.returnData,
          callData: velodromeV2FactoryIface.encodeFunctionData('token1', []),
          decodeFunction: addressDecode,
        },
        {
          target: poolResult.returnData,
          callData: velodromeV2FactoryIface.encodeFunctionData('reserve0', []),
          decodeFunction: uint256ToBigInt,
        },
        {
          target: poolResult.returnData,
          callData: velodromeV2FactoryIface.encodeFunctionData('reserve1', []),
          decodeFunction: uint256ToBigInt,
        },
      );
    }
    ...
    for (const token of tokens) {
      decimalsCalldata.push({
        target: token,
        callData: velodromeV2FactoryIface.encodeFunctionData('decimals', []),
        decodeFunction: uint256DecodeToNumber,
      });
    }
