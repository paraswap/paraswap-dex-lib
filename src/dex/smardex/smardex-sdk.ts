...
export interface CurrencyAmount {
  ...
  forcedPriceAverageTimestamp?: BigNumber;
}
 ...
 * @returns {BigNumber[]} [amountOut, newResIn, newResOut, newResInFic, newResOutFic]
 */
 ...
 * @param {BigNumber} feesLP LP fees
 * @param {BigNumber} feesPool Pool fees
 * @returns {BigNumber[]} [amountIn, newResIn, newResOut, newResInFic, newResOutFic]
 */
...
export function computeAmountOut(
  ...
  priceAverageLastTimestamp: BigNumber,
  priceAverage0: BigNumber,
  priceAverage1: BigNumber,
  feesLP: BigNumber,
  feesPool: BigNumber,
  forcedPriceAverageTimestamp: BigNumber = BigNumber.from(
    Math.ceil(Date.now() / 1000) + LATENCY_OFFSET_SECONDS,
  ),
  maxBlockDiffSeconds = BigNumber.from(300),
): CurrencyAmount {
 ...
 * @param {BigNumber} priceAverageLastTimestamp last timestamp in seconds of price average values.
 ...
 * @param {BigNumber} currentTimestampInSecond current timestamp in seconds.
 * @param {BigNumber} maxBlockDiffSeconds: Max block difference in seconds
...
export function getUpdatedPriceAverage(
  ...
  priceAverageIn: BigNumber,
  priceAverageOut: BigNumber,
  currentTimestampInSecond: BigNumber,
  maxBlockDiffSeconds: BigNumber,
): [BigNumber, BigNumber] {
  if (currentTimestampInSecond.lt(priceAverageLastTimestamp)) {
    throw new Error('INVALID_TIMESTAMP');
  }

  // very first time
  if (
    priceAverageLastTimestamp.eq(0) ||
    priceAverageIn.eq(0) ||
  ...
  if (priceAverageLastTimestamp.eq(currentTimestampInSecond)) {
    return [priceAverageIn, priceAverageOut];
  }

  // need to compute new linear-average price
  // compute new price:
  const timeDiff = currentTimestampInSecond
    .sub(priceAverageLastTimestamp)
    .lte(maxBlockDiffSeconds)
    ? currentTimestampInSecond.sub(priceAverageLastTimestamp)
    : maxBlockDiffSeconds;
  ...
  const priceAverageOutRet = priceAverageOut
    ...
    .mul(maxBlockDiffSeconds.sub(timeDiff))
    .div(priceAverageIn)
    .add(reserveFicOut.mul(timeDiff))
    .div(maxBlockDiffSeconds);
