...
export class DystopiaVolatile extends UniswapV2 {
  /// 0.05% swap fee
  ...
  async getSellPrice(
    ...
    const amountInWithFee =
      srcAmount - srcAmount / DystopiaVolatile.SWAP_FEE_FACTOR;

    const numerator = amountInWithFee * BigInt(reservesOut);

    const denominator =
      BigInt(reservesIn) * DystopiaVolatile.SWAP_FEE_FACTOR + amountInWithFee;
  ...
  async getBuyPrice(
    ...
    const numerator =
      BigInt(reservesIn) * destAmount * DystopiaVolatile.SWAP_FEE_FACTOR;
    const denominator =
      (DystopiaVolatile.SWAP_FEE_FACTOR - 1n) *
