...
export class ConeUniswapV2Pool {
  ...
  static async getSellPrice(
    ...
    const amountInWithFee = srcAmount * (SWAP_FEE_BASE_BN - 1n);

    const numerator = amountInWithFee * BigInt(reservesOut);

    const denominator = BigInt(reservesIn) * SWAP_FEE_BASE_BN + amountInWithFee;
  ...
  static async getBuyPrice(
    ...
    const numerator = BigInt(reservesIn) * destAmount * SWAP_FEE_BASE_BN;
    const denominator =
      (SWAP_FEE_BASE_BN - 1n) * (BigInt(reservesOut) - destAmount);
