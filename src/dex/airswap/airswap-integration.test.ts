...
/*
  ...
  This test script adds tests for AirSwap general integration
  with the DEX interface. The test cases below are example tests.
  It is recommended to add tests which cover AirSwap specific
  logic.
...
async function checkOnChainPricing(
  ...
  const readerResult = (
    await airswap.dexHelper.multiContract.methods
...
async function testPricingOnNetwork(
  ...
  if (airswap.hasConstantPriceLargeAmounts) {
...
describe('AirSwap', function () {
  ...
  describe('Mainnet', () => {
    ...
    it('getTopPoolsForToken', async function () {
      ...
      if (newAirSwap.updatePoolState) {
        await newAirSwap.updatePoolState();
      }
      const poolLiquidity = await newAirSwap.getTopPoolsForToken(
        tokens[srcTokenSymbol].address,
        10,
      );
      console.log(`${srcTokenSymbol} Top Pools:`, poolLiquidity);

      if (!newAirSwap.hasConstantPriceLargeAmounts) {
        checkPoolsLiquidity(
          poolLiquidity,
