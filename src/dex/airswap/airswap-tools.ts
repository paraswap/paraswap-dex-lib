...
export async function getStakersUrl(provider: providers.Provider, makerAddress: string) {
...
export async function getMakersLocatorForTX(provider: providers.Provider,
    from: Token,
    to: Token,
    network: number) {
    const pairs = await getAvailableMakersForRFQ(provider, from, to, network);
    const keys = pairs.map(
        ({ swapContract, locator }) => ({ swapContract, url:locator })
    );
    return Promise.resolve(keys);
}

export async function getTx(host: string, swapContract: string, senderWallet: string,     
    srcToken: Token, destToken: Token, amount: string) {
