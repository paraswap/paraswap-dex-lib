...
export class PancakeswapV3EventPool extends StatefulEventSubscriber<PoolState> {
  ...
  protected processLog(
    ...
    try {
      ...
      const pancakeswapV3EventLoggingSampleRate =
        this.dexHelper.config.data.pancakeswapV3EventLoggingSampleRate;
      if (
        !this.dexHelper.config.isSlave &&
        pancakeswapV3EventLoggingSampleRate &&
        isSampled(pancakeswapV3EventLoggingSampleRate)
      ) {
        this.logger.info(
          `event=${event.name} - block=${
            blockHeader.number
          }. Log sampled at rate ${pancakeswapV3EventLoggingSampleRate * 100}%`,
        );
      }
          ...
          if (
            ...
            this.logger.error(
              `${this.parentName}: Pool ${this.poolAddress}, ` +
                ...
                } for PancakeswapV3, ${JSON.stringify(event)}`,
              e,
            );
          }
  ...
  async generateState(blockNumber: number): Promise<Readonly<PoolState>> {
    ...
    // I think PancakeswapV3 callbacks subscriptions are complexified for no reason.
  ...
  handleBurnEvent(
    ...
    // And it aligns with PancakeswapV3 doc:
    // https://github.com/Uniswap/v3-core/blob/05c10bf6d547d6121622ac51c457f93775e1df09/contracts/interfaces/pool/IPancakeswapV3PoolActions.sol#L59
