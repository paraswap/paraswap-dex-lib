...
import PancakeswapV3QuoterABI from '../../abi/pancakeswap-v3/PancakeswapV3QuoterV2.abi.json';
import PancakeswapV3MultiABI from '../../abi/pancakeswap-v3/PancakeswapMulti.abi.json';
import PancakeswapV3StateMulticallABI from '../../abi/pancakeswap-v3/PancakeswapV3StateMulticall.abi.json';
import {
  PANCAKESWAPV3_EFFICIENCY_FACTOR,
  PANCAKESWAPV3_FUNCTION_CALL_GAS_COST,
...
export class PancakeswapV3
  ...
  implements IDex<PancakeswapV3Data, PancakeswapV3Param>
{
  ...
  public static dexKeysWithNetwork: { key: string; networks: Network[] }[] =
    getDexKeysWithNetwork(PancakeswapV3Config);
  ...
  constructor(
    ...
    this.uniswapMulti = new this.dexHelper.web3Provider.eth.Contract(
      PancakeswapV3MultiABI as AbiItem[],
      this.config.pancakeswapMulticall,
    );
  ...
  ): Promise<ExchangePrices<PancakeswapV3Data> | null> {
    if (pools.length === 0) {
      return null;
    }
  ...
  async getPricesVolume(
    ...
    blockNumber: number,
    limitPools?: string[],
  ): Promise<null | ExchangePrices<PancakeswapV3Data>> {
    try {
      const _srcToken = this.dexHelper.config.wrapETH(srcToken);
      const _destToken = this.dexHelper.config.wrapETH(destToken);
      ...
      const notNullResult = result.filter(
        res => res !== null,
      ) as ExchangePrices<PancakeswapV3Data>;
  ...
  getCalldataGasCost(
    poolPrices: PoolPrices<PancakeswapV3Data>,
  ): number | number[] {
    const gasCost =
  ...
  async getSimpleParam(
    ...
    const swapFunction =
      side === SwapSide.SELL
        ? PancakeswapV3Functions.exactInput
        : PancakeswapV3Functions.exactOutput;

    const path = this._encodePath(data.path, side);
    const swapFunctionParams: PancakeswapV3Param =
      side === SwapSide.SELL
        ? {
  ...
  private _toLowerForAllConfigAddresses() {
    ...
    const newConfig: DexParams = {
      ...
      pancakeswapMulticall: this.config.pancakeswapMulticall,
