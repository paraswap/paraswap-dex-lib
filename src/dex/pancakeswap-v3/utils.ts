...
export function getPancakeswapV3DexKey(
  PancakeswapV3Config: DexConfigMap<DexParams>,
) {
  const PancakeswapV3Keys = Object.keys(PancakeswapV3Config);
  if (PancakeswapV3Keys.length !== 1) {
    throw new Error(
      `PancakeswapV3 key in PancakeswapV3Config is not unique. Update relevant places (optimizer) or fix config issue. Received: ${JSON.stringify(
        PancakeswapV3Config,
  ...
  return PancakeswapV3Keys[0].toLowerCase();
}
