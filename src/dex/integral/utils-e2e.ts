...
if (fs.existsSync(absConfigPath)) configs = require(absConfigPath);

const statePath = './states.json';
const absStatePath = path.join(pathRoot, statePath);
let states: { [k: string]: any } = {};
if (fs.existsSync(absStatePath)) states = require(absStatePath);

const logPath = './logs.json';
const absLogPath = path.join(pathRoot, logPath);
let logs: { [k: string]: BlockInfo } = {};
if (fs.existsSync(absLogPath)) logs = require(absLogPath);
...
export async function updateEventSubscriber<SubscriberState>(
...
export async function checkEventSubscriber<SubscriberState>(
  eventSubscriber: StatefulEventSubscriber<SubscriberState>,
  fetchState: (blocknumber: number) => Promise<SubscriberState>,
  blockNumber: number,
  cacheKey: string,
