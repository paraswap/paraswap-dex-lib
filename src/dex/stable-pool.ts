...
export class StablePool extends SimpleExchange
  implements IDex<StablePoolData, StablePoolParam> {
    ...
    getSimpleParam(srcToken: string, destToken: string, srcAmount: string, destAmount: string, data: StablePoolData, side: SwapSide): SimpleExchangeParam {
      if(side !== SwapSide.BUY) throw BUY_NOT_SUPPORTED_ERRROR

      const { exchange, i, j, deadline } = data;
      const defaultArgs = [i, j, srcAmount, this.minConversionRate, deadline];
      const swapData = this.exchangeRouterInterface.encodeFunctionData(StablePoolSwapMethod, defaultArgs);
